...
##
##   - 'X-Varnish-Hash-Extra' header can be used to include additional input
##     (e.g., normalized cookie values) for the caching key generation.
##
...
sub vcl_synth {
    ...
    # Inject debug headers in the client response? Some of them are commented
    # out because the same info was previously unconditionally injected as
    # informational headers.
    if (req.http.X-Varnish-Debug) {
        set resp.http.X-Varnish-Debug-Hash-Data = req.http.X-Varnish-Debug-Hash-Data;
...
sub vcl_hash {
    ...
    if (req.http.X-Varnish-Bodyaccess-Method) {
        ...
        set req.http.X-Varnish-Debug-Hash-Data = req.http.X-Varnish-Debug-Hash-Data +
            "[body:" + req.http.X-Varnish-Bodyaccess-Method + "]";
    } else {
        hash_data("");
        hash_data("");
    }

    # Same for additional hashing input generated by the route-specific 'vcl_recv'
    # and 'vcl_hash' subroutines.
    if (req.http.X-Varnish-Hash-Extra) {
        hash_data(req.http.X-Varnish-Hash-Extra);
        set req.http.X-Varnish-Debug-Hash-Data = req.http.X-Varnish-Debug-Hash-Data +
            "[extra:" + req.http.X-Varnish-Hash-Extra + "]";
    } else {
        hash_data("");
    }
    ...
    if (req.http.X-Varnish-Hash-Url) {
        ...
        set req.http.X-Varnish-Debug-Hash-Data = req.http.X-Varnish-Debug-Hash-Data +
            "[url:" + req.http.X-Varnish-Hash-Url + "]";
    } else {
        hash_data(req.url);
        set req.http.X-Varnish-Debug-Hash-Data = req.http.X-Varnish-Debug-Hash-Data +
            "[url:" + req.url + "]";
    ...
    if (req.http.X-Varnish-Hash-Host) {
        ...
        set req.http.X-Varnish-Debug-Hash-Data = req.http.X-Varnish-Debug-Hash-Data +
            "[host:" + req.http.X-Varnish-Hash-Host + "]";
    } elsif (req.http.Host) {
...
sub vcl_deliver {
    ...
    #
    # Beware both self-routed cluster requests and Akamai requests will disable
    # ESI processing setting 'resp.do_esi' to 'false' before reaching this point.
    if (obj.can_esi && resp.do_esi && req.esi_level == 0) {
